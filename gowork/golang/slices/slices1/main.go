...
func getMessageWithRetries(primary, secondary, tertiary string) ([3]string, [3]int) {
	msg_collections := [3]string{primary, secondary, tertiary}
	msg_cost := [3]int{len(primary), len(secondary) + len(primary), len(tertiary) + len(secondary) + len(primary)}
	return msg_collections, msg_cost

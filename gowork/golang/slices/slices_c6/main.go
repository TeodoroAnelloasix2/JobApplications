...
In the example above, the selfMath function takes in a function as its parameter and returns a function that itself
returns the value of running that input function on its parameter.
Assignment
...
func getLogger(formatter func(string, string) string) func(string, string) {
	...
	return func(s, s1 string) {
		fmt.Println(formatter(s, s1))
	}
}
func formatter(s1, s2 string) string {
	s_formated := fmt.Sprintf("%s%s", s1, s2)
	return s_formated
}

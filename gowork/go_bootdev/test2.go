...
func placeOrder(productID string, quantity int, accountBalance float64) (bool, float64) {
	...
	disponibilidad := amountInStock(productID)

	if quantity > disponibilidad {
	...
	if quantity <= disponibilidad {
		...
		coste_pedido := calcPrice(productID, quantity)

		if coste_pedido <= accountBalance {
			accountBalance = accountBalance - coste_pedido
